...
class BotController(QObject):
    ...
    def toggle_shadow_mode(self, enabled: bool):
        """Toggle shadow mode with safety confirmation"""
        self.shadow_mode = enabled
        mode = "SHADOW MODE (Safe)" if enabled else "⚠️ LIVE TRADING (Real Money)"
        self.signal_log.emit(f"Mode changed to: {mode}", "INFO")
        
        if not enabled:
            self.signal_log.emit("⚠️ LIVE TRADING ENABLED - REAL MONEY AT RISK!", "WARNING")
